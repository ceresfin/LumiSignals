...
class DemoTradingBot:
    ...
    def __init__(self, max_risk_usd: float = 10.0, max_open_trades: int = 5):
        ...
        print("🤖 Demo Trading Bot Initialized with Fixed Dollar Risk, Enhanced Tracking & Polars!")
    ...
    def setup_logging(self):
        ...
        try:
            ...
            self.logger.info("Demo Trading Bot started with Fixed Dollar Risk Management, Enhanced Tracking & Polars")
    ...
    def get_liquidity_score(self, instrument: str) -> int:
        ...
        try:
            ...
            # Major pairs during extended optimal windows
            ...
            # If timing is optimal but not in peak periods
            if is_optimal:
                return 3
    ...
    def should_trade_instrument(self, instrument: str, signal_confidence: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if liquidity_score >= 4:
                ...
                min_confidence = 75  # Slightly lower for medium liquidity
            elif liquidity_score >= 2:
                min_confidence = 80  # Standard threshold
            else:
                min_confidence = 85  # Higher threshold during lower liquidity
            
            # Allow trading if confidence is sufficient, even if timing isn't perfect
            confidence_sufficient = signal_confidence >= min_confidence
            
            # For very high confidence signals, be more flexible with timing
            if signal_confidence >= 90:
                min_confidence = max(70, min_confidence - 10)
            ...
            # If confidence is good but timing is poor, still consider if liquidity is decent
    ...
    def scan_for_opportunities(self) -> List[TradeOrder]:
        """Scan all instruments for trading opportunities - ENHANCED DEBUG VERSION"""
        opportunities = []
        
        self.logger.info("🔍 Scanning for trading opportunities with enhanced debugging...")
        ...
        # Test a few key instruments first for debugging
        test_instruments = ['EUR_USD', 'GBP_USD', 'USD_CAD']
        
        for instrument in test_instruments:
            try:
                self.logger.info(f"\n🔍 Analyzing {instrument} in detail...")
                
                # Check timing first
                is_optimal, timing_reason = self.is_optimal_trading_time(instrument)
                liquidity_score = self.get_liquidity_score(instrument)
                
                self.logger.info(f"   Timing: {'✅' if is_optimal else '❌'} {timing_reason}")
                self.logger.info(f"   Liquidity Score: {liquidity_score}/5")
                ...
                self.logger.info(f"   Signal: {signals['action']} (Confidence: {signals['confidence']}%)")
                
                if signals['action'] != 'WAIT':
                    # Log detailed reasoning
                    self.logger.info(f"   Reasoning: {'; '.join(signals['reasoning'])}")
                    
                    # Check if timing is optimal for this instrument
                    should_trade, trade_reason = self.should_trade_instrument(
                        instrument, signals['confidence']
                    )
                    
                    self.logger.info(f"   Trade Decision: {'✅ TRADE' if should_trade else '❌ SKIP'} - {trade_reason}")
                    ...
                    if should_trade:
                        ...
                        trade_order = TradeOrder(
                            ...
                            reasoning=signals['reasoning'] + [f"Market timing: {trade_reason}"],
                        ...
                        self.logger.info(f"🎯 {instrument}: OPPORTUNITY ADDED!")
                else:
                    self.logger.info(f"   No signal - momentum not sufficient")
        ...
        # If test instruments work, scan all others
        if opportunities:
            self.logger.info(f"✅ Found {len(opportunities)} opportunities in test run. Scanning all instruments...")
        else:
            self.logger.warning("❌ No opportunities found in test instruments. Check strategy parameters.")
        
        # Scan remaining instruments
        remaining_instruments = [inst for inst in self.instruments if inst not in test_instruments]
        
        for instrument in remaining_instruments:
            try:
                # Quick analysis for remaining instruments
                analysis = self.strategy.analyze_penny_curve_setup(instrument)
        ...
        self.logger.info(f"📊 Total opportunities found: {len(opportunities)}")
    ...
    def generate_trading_report(self) -> str:
        ...
        try:
            ...
            report = []
            report.append("="*80)
            report.append("DEMO TRADING BOT - STATUS REPORT (Fixed Dollar Risk + Enhanced Tracking + Polars)")
            report.append("="*80)
            ...
            # Polars integration status
            report.append("")
            report.append("🚀 TECHNICAL STATUS:")
            report.append(f"   Data Processing: {'Polars (AWS Lambda Ready)' if POLARS_AVAILABLE else 'Pandas (Local Only)'}")
            report.append(f"   Market Timing: Enhanced with session overlap detection")
            report.append(f"   Risk Management: Fixed dollar amount with confidence scaling")
    ...
    def start_trading(self) -> None:
        ...
        print("🚀 Starting Demo Trading Bot with Fixed Dollar Risk, Enhanced Tracking & Polars!")
    ...
    def run_single_cycle(self) -> None:
        ...
        print("🧪 Running single trading cycle with fixed dollar risk, enhanced tracking & Polars...")
        self.trading_cycle()

def main():
    """Main function to run the demo trading bot"""
    print("🤖 Demo Trading Bot for Penny Curve Strategy with Fixed Dollar Risk, Enhanced Tracking & Polars")
    print("="*80)
