...
class CPAPIClient:
    ...
    def build_combo_order(legs, quantity: int, limit_price: float,
        ...
        That bug led the ORB butterfly handler to place each leg as a
        separate single-leg order, which exposes us to partial-fill
        tail risk on naked option legs. Use THIS helper for any new
        spread placement.
    ...
    def build_spread_order(sell_conid: int, buy_conid: int, quantity: int,
        ...
        compatibility with the older call sites that pass leg conids
