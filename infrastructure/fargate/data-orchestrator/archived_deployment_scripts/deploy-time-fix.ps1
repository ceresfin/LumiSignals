...
# Usage: .\deploy-time-fix.ps1

Write-Host "🕐 LumiSignals Time Handling Fix Deployment" -ForegroundColor Green
...
Write-Host "   ✅ Parse OANDA nanosecond timestamps (*.000000000Z)" -ForegroundColor White
...
Write-Host "🏗️ Building Docker image with time handling fixes..." -ForegroundColor Cyan
docker build `
...
# Step 3: Tag for ECR
Write-Host "🏷️ Tagging images for ECR..." -ForegroundColor Cyan
docker tag ${ECR_REPOSITORY}:$UNIQUE_HASH ${ECR_URI}:$UNIQUE_HASH
docker tag ${ECR_REPOSITORY}:latest ${ECR_URI}:latest

# Step 4: Push to ECR
Write-Host "☁️ Pushing images to ECR..." -ForegroundColor Cyan
...
Write-Host "✍️ Registering new task definition..." -ForegroundColor Cyan
...
Write-Host "   ✅ Added _parse_oanda_timestamp() method" -ForegroundColor White
Write-Host "   ✅ Handle .000000000Z nanosecond precision" -ForegroundColor White
Write-Host "   ✅ Proper datetime sorting in H1 backfill" -ForegroundColor White
...
Write-Host ""
Write-Host "Monitor H1 backfill and data collection:" -ForegroundColor Cyan
