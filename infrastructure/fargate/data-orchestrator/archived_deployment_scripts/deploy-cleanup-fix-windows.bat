...
REM Use existing built image with comprehensive orchestrator fix
set IMAGE_TAG=816945674467.dkr.ecr.us-east-1.amazonaws.com/lumisignals/institutional-orchestrator-postgresql17:final-fix-20250908-221833
set LATEST_TAG=816945674467.dkr.ecr.us-east-1.amazonaws.com/lumisignals/institutional-orchestrator-postgresql17:latest

echo [1/7] Using pre-built container with comprehensive orchestrator fix...
echo Image tag: %IMAGE_TAG%
echo Skipping build - using existing container
...
for /f "tokens=*" %%i in ('aws ecr describe-images --repository-name lumisignals/institutional-orchestrator-postgresql17 --image-ids imageTag^=final-fix-20250908-221833 --region us-east-1 --query "imageDetails[0].imageDigest" --output text') do set IMAGE_DIGEST=%%i
...
aws logs filter-log-events --log-group-name /ecs/lumisignals-data-orchestrator --region us-east-1 --start-time %LOG_START_TIME% --query "events[].message" --output text | findstr /C:"comprehensive" /C:"🎯" /C:"Comprehensive orchestrator initialized"
...
echo 2. Deployment method: stop/start with force refresh
echo 3. Wait time: 4 minutes (240 seconds)
echo 4. Log verification: automated search completed
echo.
echo NEXT DATA CYCLE:
echo - Data collection happens every 5 minutes
echo - Trade 1581 cleanup should happen automatically
echo - Check pipstop.org in 10-15 minutes to verify
echo.
goto :end
