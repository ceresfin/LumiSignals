...
try:
    ...
    # Fixed: Use metadata_cache instead of _metadata_cache
    print(f"SUCCESS: Initialized enhanced metadata store with {len(metadata_store.metadata_cache)} entries")
except ImportError as e:
    print(f"WARNING: Could not import enhanced metadata storage: {e}")
    print("Falling back to original metadata parsing only")
    metadata_store = None
except AttributeError as e:
    print(f"WARNING: Enhanced metadata store initialization issue: {e}")
    print("Using basic metadata store functionality")
    metadata_store = None
...
def calculate_risk_metrics(entry_price, stop_loss_price, target_price, units, account_balance=None):
    ...
    FIXED: Proper string formatting for logging
    """
    metrics = {
        'risk_pips': None,
        'reward_pips': None,
    ...
    try:
        ...
        # FIXED: Safe logging with proper formatting
        rr_display = f"{metrics['rr_ratio']:.2f}" if metrics.get('rr_ratio') is not None else 'N/A'
        logger.info(f"📊 Risk metrics: Risk={risk_pips:.1f}pips, Reward={reward_pips:.1f}pips, R:R={rr_display}")
        return metrics
...
# --- FIXED: Airtable Select Field Mapping Functions ---
def map_momentum_direction_to_airtable(direction_str):
    """Map Quarter Curve directions to exact Airtable select options"""
    if not direction_str:
        return None
    
    # Map to exact Airtable options based on the field configuration
    direction_mapping = {
    ...
    direction = str(direction_str).upper()
    mapped = direction_mapping.get(direction, 'NEUTRAL')
    logger.info(f"🎯 Momentum Direction mapped: '{direction}' -> '{mapped}'")
    return mapped

def map_strategy_bias_to_airtable(bias_str):
    """Map strategy bias to exact Airtable select options"""
    if not bias_str:
        return None
    ...
    bias = str(bias_str).upper()
    mapped = bias_mapping.get(bias, 'NEUTRAL')
    logger.info(f"🎯 Strategy Bias mapped: '{bias}' -> '{mapped}'")
    return mapped
...
# --- FIXED: Enhanced Metadata Storage ---
def store_enhanced_metadata_safely(metadata_store, order_id, enhanced_metadata):
    """Safely store enhanced metadata handling dict conversion"""
    try:
        if metadata_store and hasattr(metadata_store, 'store_enhanced_metadata'):
            # Convert dict to proper metadata object if needed
            if isinstance(enhanced_metadata, dict):
                try:
                    from metadata_storage import EnhancedTradeMetadata
                    metadata_obj = EnhancedTradeMetadata(**enhanced_metadata)
                    metadata_store.store_enhanced_metadata(order_id, metadata_obj)
                except Exception as conv_error:
                    logger.warning(f"Could not convert to EnhancedTradeMetadata: {conv_error}")
                    # Fall back to regular storage
                    if hasattr(metadata_store, 'store_metadata'):
                        metadata_store.store_metadata(order_id, enhanced_metadata)
            else:
                metadata_store.store_enhanced_metadata(order_id, enhanced_metadata)
        elif metadata_store and hasattr(metadata_store, 'store_metadata'):
            metadata_store.store_metadata(order_id, enhanced_metadata)
        else:
            logger.info(f"✅ Enhanced metadata prepared for order {order_id}")
            
    except Exception as e:
        logger.warning(f"Could not store enhanced metadata: {e}")
        logger.info(f"✅ Enhanced metadata prepared for order {order_id}")
...
def create_airtable_fields_with_enhanced_prices(base_fields, metadata, price_levels, risk_metrics=None):
    ...
    FIXED: Proper mapping to exact Airtable select options
    """
    ...
    # Add cleaned metadata fields with FIXED mappings
    for metadata_key, airtable_field in metadata_field_mapping.items():
        value = cleaned_metadata.get(metadata_key)
        
        # Special handling for select fields that need exact matching
        if metadata_key == 'zone_position':
            mapped_zone = map_quarter_curve_zone_position(
                value, 
                cleaned_metadata.get('strategy_tag'),
                None,  # candlestick_strength
                None   # breach_direction
            )
            fields[airtable_field] = mapped_zone
            logger.info(f"🎯 Zone Position mapped: '{value}' -> '{mapped_zone}'")
        elif metadata_key == 'momentum_direction':
            # FIXED: Map to exact Airtable options
            mapped_direction = map_momentum_direction_to_airtable(value)
            if mapped_direction:
                fields[airtable_field] = mapped_direction
        elif metadata_key == 'strategy_bias':
            # FIXED: Map to exact Airtable options  
            mapped_bias = map_strategy_bias_to_airtable(value)
            if mapped_bias:
                fields[airtable_field] = mapped_bias
        else:
    ...
    # FIXED: Handle Momentum Strength as percentage (0-1 for Airtable)
    if cleaned_metadata.get('momentum_strength') is not None:
        strength_value = cleaned_metadata['momentum_strength']
        if isinstance(strength_value, (int, float)):
            # Ensure the value is between 0 and 1 for Airtable percentage field
            if strength_value > 1:
                # If it's a percentage like 75, convert to 0.75
                fields['Momentum Strength'] = strength_value / 100
            else:
                # Already in decimal format (0.75 = 75%)
                fields['Momentum Strength'] = strength_value
        else:
            # Try to parse string percentage
            try:
                str_val = str(strength_value).replace('%', '')
                numeric_val = float(str_val)
                if numeric_val > 1:
                    fields['Momentum Strength'] = numeric_val / 100
                else:
                    fields['Momentum Strength'] = numeric_val
            except:
                # Fallback to 0 if can't parse
                fields['Momentum Strength'] = 0.0
    ...
    try:
        ...
        if order_id:
            try:
                ...
                # metadata_store might not have get_order_metadata method
                pass
...
def convert_local_to_metadata_dict(local_metadata):
    ...
    try:
        ...
        if hasattr(local_metadata, 'get'):
            ...
            converted = {
                'setup_name': getattr(local_metadata, 'setup_name', None),
                'momentum_strength': getattr(local_metadata, 'momentum_strength', None),
                'momentum_direction': getattr(local_metadata, 'momentum_direction', None),
                'strategy_bias': getattr(local_metadata, 'strategy_bias', None),
                'zone_position': getattr(local_metadata, 'zone_position', None),
                'distance_pips': getattr(local_metadata, 'distance_to_entry_pips', None),
                'confidence': getattr(local_metadata, 'signal_confidence', None),
                'alignment_score': getattr(local_metadata, 'momentum_alignment', None),
                'strategy_tag': getattr(local_metadata, 'strategy_tag', None)
            }
        
        # Special handling for Quarter Curve Butter Middle strategy
        setup_name = converted.get('setup_name', '')
        if setup_name and 'QCButterMiddle' in setup_name:
            logger.info(f"🎯 Processing Quarter Curve setup: {setup_name}")
            
            # Parse candlestick strength and breach direction from setup name
            parts = setup_name.split('_')
            if len(parts) >= 5:
                # Extract strength from the last part
                strength_part = parts[-1]  # e.g., "bearish" or "strong_bearish"
                
                # Map Quarter Curve strength to Airtable values
                if 'strong_bearish' in strength_part:
                    converted['momentum_direction'] = 'STRONG_BEARISH'
                    converted['momentum_strength'] = 0.85
                    converted['strategy_bias'] = 'BEARISH'
                elif 'bearish' in strength_part:
                    converted['momentum_direction'] = 'WEAK_BEARISH'
                    converted['momentum_strength'] = 0.75
                    converted['strategy_bias'] = 'BEARISH'
                elif 'strong_bullish' in strength_part:
                    converted['momentum_direction'] = 'STRONG_BULLISH'
                    converted['momentum_strength'] = 0.85
                    converted['strategy_bias'] = 'BULLISH'
                elif 'bullish' in strength_part:
                    converted['momentum_direction'] = 'WEAK_BULLISH'
                    converted['momentum_strength'] = 0.75
                    converted['strategy_bias'] = 'BULLISH'
                
                # Extract action (BUY/SELL) from parts
                if 'SELL' in parts:
                    converted['strategy_bias'] = 'BEARISH'
                elif 'BUY' in parts:
                    converted['strategy_bias'] = 'BULLISH'
                
                # Set Quarter Curve specific zone position
                converted['zone_position'] = 'In_Sell_Zone' if 'SELL' in parts else 'In_Buy_Zone'
                
                logger.info(f"🎯 Extracted Quarter Curve metadata:")
                logger.info(f"   Direction: {converted['momentum_direction']}")
                logger.info(f"   Strength: {converted['momentum_strength']}")
                logger.info(f"   Bias: {converted['strategy_bias']}")
                logger.info(f"   Zone: {converted['zone_position']}")
...
def enhanced_process_order_fill_with_prices(tx):
    ...
    try:
        ...
        if "tradeOpened" in tx:
            ...
            # Convert metadata for Airtable with FIXED mappings
            metadata_for_airtable = {
                ...
                'momentum_direction': metadata.get('momentum_direction'),  # Will be mapped properly
                'strategy_bias': metadata.get('strategy_bias'),  # Will be mapped properly
            ...
            # FIXED: Store enhanced metadata safely
            enhanced_metadata = {
                'setup_name': setup_name,
            ...
            store_enhanced_metadata_safely(metadata_store, order_id, enhanced_metadata)
...
def enhanced_sync_new_fills():
    ...
    if metadata_store and hasattr(metadata_store, 'cleanup_old_metadata'):
        try:
            metadata_store.cleanup_old_metadata()
        except Exception as e:
            logger.warning(f"Could not cleanup old metadata: {e}")
...
def enhanced_main():
    ...
    if metadata_store:
        try:
            if hasattr(metadata_store, 'get_metadata_stats'):
                ...
                logger.info(f"📊 Enhanced metadata store loaded: {len(metadata_store.metadata_cache)} entries")
        except Exception as e:
            logger.warning(f"Could not get metadata store stats: {e}")
    else:
        ...
        if metadata_store:
            try:
                if hasattr(metadata_store, 'get_metadata_stats'):
                    ...
                    logger.info(f"📊 Final enhanced metadata store: {len(metadata_store.metadata_cache)} entries")
            except Exception as e:
                logger.warning(f"Could not get final metadata store stats: {e}")
    else:
    ...
    try:
        ...
        for record in records:
            try:
                ...
                if metadata_store and order_id and hasattr(metadata_store, 'get_order_metadata'):
                    try:
                        local_metadata = metadata_store.get_order_metadata(order_id)
                        if local_metadata:
