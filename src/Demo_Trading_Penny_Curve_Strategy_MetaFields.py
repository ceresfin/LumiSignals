...
class WeekendTestingConfig:
    """Configuration for weekend testing mode"""
    
    def __init__(self, enable_weekend_testing: bool = False):
        self.enable_weekend_testing = enable_weekend_testing
        self.simulated_market_time = None
        self.simulated_session = None
    
    def set_simulated_time(self, hour: int, weekday: int = 2):
        """
        Set simulated market time for testing
        hour: 0-23 (ET/EDT)
        weekday: 0=Monday, 1=Tuesday, etc. (2=Wednesday default)
        """
        ...
        self.simulated_market_time = base_date + timedelta(days=days_to_add)
        
        # Determine simulated session
        if 3 <= hour <= 11:
            if 8 <= hour <= 11:
                self.simulated_session = "London-NY Overlap"
            else:
                self.simulated_session = "London"
        elif 8 <= hour <= 17:
            self.simulated_session = "New York"
        elif 19 <= hour <= 23 or 0 <= hour <= 4:
            self.simulated_session = "Tokyo"
        else:
            self.simulated_session = "Low Liquidity"
...
class DemoTradingBot:
    """
    Demo Trading Bot for Penny Curve Strategy with Fixed Dollar Risk Management and WEEKEND TESTING MODE
    ...
    - NEW: WEEKEND TESTING MODE for strategy testing
    """
    
    def __init__(self, max_risk_usd: float = 10.0, max_open_trades: int = 5, weekend_testing: bool = False):
        ...
        try:
            ...
            # NEW: Weekend testing configuration
            self.weekend_testing = WeekendTestingConfig(weekend_testing)
            
            if weekend_testing:
                print("🧪 WEEKEND TESTING MODE ENABLED")
                print("   Market checks will be bypassed for testing")
                print("   Using simulated market conditions")
                print("   All trades will still be placed in practice account")
        ...
        print("🤖 Demo Trading Bot Initialized - ENHANCED WITH AIRTABLE METADATA + WEEKEND TESTING!")
        ...
        if weekend_testing:
            print(f"   🧪 WEEKEND TESTING MODE: Market timing bypassed")
    ...
    def setup_logging(self):
        ...
        try:
            ...
            mode_info = "WEEKEND TESTING MODE" if self.weekend_testing.enable_weekend_testing else "LIVE MODE"
            self.logger.info(f"Demo Trading Bot started with ENHANCED AIRTABLE METADATA INTEGRATION - {mode_info}")
    ...
    def set_weekend_testing_session(self, session_name: str, hour: int = None):
        """
        Set simulated trading session for weekend testing
        
        session_name: 'london', 'newyork', 'tokyo', 'overlap'
        hour: specific hour to simulate (optional)
        """
        if not self.weekend_testing.enable_weekend_testing:
            self.logger.warning("Weekend testing not enabled")
            return
        
        session_hours = {
            'london': 8,      # 8 AM ET - London session
            'newyork': 10,    # 10 AM ET - London-NY overlap
            'tokyo': 21,      # 9 PM ET - Tokyo session
            'overlap': 10     # 10 AM ET - London-NY overlap
        }
        
        target_hour = hour if hour is not None else session_hours.get(session_name.lower(), 10)
        
        self.weekend_testing.set_simulated_time(target_hour, weekday=2)  # Wednesday
        
        self.logger.info(f"🧪 Weekend testing session set: {session_name.upper()} at {target_hour:02d}:00 ET")
        self.logger.info(f"   Simulated time: {self.weekend_testing.simulated_market_time}")
        self.logger.info(f"   Simulated session: {self.weekend_testing.simulated_session}")
    
    def is_market_open(self) -> bool:
        """Check if forex market is currently open - MODIFIED FOR WEEKEND TESTING"""
        if self.weekend_testing.enable_weekend_testing:
            # Always return True during weekend testing
            return True
        
        # Original market open logic
        try:
    ...
    def get_market_time_et(self) -> datetime:
        """Get current time in ET/EDT - MODIFIED FOR WEEKEND TESTING - BULLETPROOF TIMEZONE FIX"""
        try:
            current_time = None
            
            if self.weekend_testing.enable_weekend_testing and self.weekend_testing.simulated_market_time:
                # Return simulated time for testing
                current_time = self.weekend_testing.simulated_market_time
                self.logger.debug(f"Using simulated time: {current_time}")
            elif PYTZ_AVAILABLE:
                try:
                    et_tz = pytz.timezone('US/Eastern')
                    current_time = datetime.now(et_tz)
                    self.logger.debug(f"Using pytz time: {current_time}")
                except Exception as e:
                    self.logger.debug(f"Pytz failed: {e}")
                    current_time = None
            
            # Fallback if pytz failed or not available
            if current_time is None:
                ...
                if 3 <= month <= 10:  # Rough DST period
                    ...
                    tz_suffix = "EST"
                current_time = utc_now + est_offset
                self.logger.debug(f"Using fallback time: {current_time}")
            
            # BULLETPROOF FIX: Force create a new datetime object with tz_name attribute
            # This ensures we ALWAYS have a datetime with the required attribute
            if hasattr(current_time, 'year'):  # Valid datetime object
                # Create a brand new datetime object
                new_time = datetime(
                ...
                if hasattr(current_time, 'dst') and callable(current_time.dst):
                    try:
                        tz_name = "EDT" if current_time.dst() else "EST"
                    except:
                        tz_name = "EDT"  # Default to EDT
                elif hasattr(current_time, 'tzinfo') and current_time.tzinfo:
                    try:
                        # Try to determine from tzinfo
                        tz_name = "EDT" if current_time.month in range(3, 11) else "EST"
                    except:
                        tz_name = "EDT"
                else:
                ...
                # Force set the tz_name attribute on the NEW datetime object
                new_time.tz_name = tz_name
                
                self.logger.debug(f"Created bulletproof time with tz_name: {new_time}, tz_name: {tz_name}")
                return new_time
            else:
                # Emergency fallback
                now = datetime.now()
            ...
            self.logger.error(f"Error in get_market_time_et: {e}")
            # Emergency fallback
            emergency_time = datetime.now()
            emergency_time.tz_name = "EDT"
            return emergency_time
    
    def get_trading_session_info(self) -> Dict:
        """Get enhanced session information - MODIFIED FOR WEEKEND TESTING"""
        if self.weekend_testing.enable_weekend_testing:
            # Return simulated session info
            simulated_time = self.weekend_testing.simulated_market_time or datetime.now()
            
            return {
                'current_time_et': simulated_time.strftime('%Y-%m-%d %H:%M:%S') + " EDT (SIMULATED)",
                'active_sessions': [self.weekend_testing.simulated_session or "London"],
                'overlaps': ['London-NY'] if self.weekend_testing.simulated_session == "London-NY Overlap" else [],
                'liquidity_level': 'HIGH'  # Assume high liquidity for testing
            }
    ...
    def is_optimal_trading_time(self, instrument: str) -> Tuple[bool, str]:
        """Check if current time is optimal for trading - MODIFIED FOR WEEKEND TESTING"""
        if self.weekend_testing.enable_weekend_testing:
            # During weekend testing, assume optimal timing
            return True, "Weekend testing mode - optimal timing simulated"
        ...
        try:
            ...
            # Check instrument-specific optimal window with correct logic
    ...
    def should_trade_instrument(self, instrument: str, signal_confidence: int) -> Tuple[bool, str]:
        """Determine if we should trade an instrument - MODIFIED FOR WEEKEND TESTING"""
        if self.weekend_testing.enable_weekend_testing:
            # During weekend testing, use relaxed confidence requirements
            min_confidence = 60  # Lower threshold for testing
            
            if signal_confidence >= min_confidence:
                return True, f"Weekend testing: Confidence {signal_confidence}% >= {min_confidence}%"
            else:
                return False, f"Weekend testing: Confidence {signal_confidence}% < {min_confidence}%"
        ...
        try:
            ...
            # More realistic confidence requirements
    ...
    def create_trade_metadata(self, trade_order: TradeOrder, momentum_analysis: Dict, zone_data: Dict) -> TradeMetadata:
        ...
        try:
            ...
            # Add weekend testing context if active
            notes = f"Reasoning: {'; '.join(trade_order.reasoning)}"
            if self.weekend_testing.enable_weekend_testing:
                notes += f" | WEEKEND TESTING MODE - Simulated session: {self.weekend_testing.simulated_session}"
            ...
            if self.weekend_testing.enable_weekend_testing:
                self.logger.info(f"   🧪 Weekend Testing: {self.weekend_testing.simulated_session}")
    ...
    def get_liquidity_score(self, instrument: str) -> int:
        """Get liquidity score for instrument at current time (1-5, 5 = highest liquidity) - MODIFIED FOR WEEKEND TESTING"""
        if self.weekend_testing.enable_weekend_testing:
            # Return high liquidity score for testing
            return 4
    ...
    def scan_for_opportunities(self) -> List[TradeOrder]:
        ...
        mode_info = "🧪 WEEKEND TESTING" if self.weekend_testing.enable_weekend_testing else "LIVE"
        self.logger.info(f"🔍 Scanning for trading opportunities ({mode_info}) with fixed dollar risk...")
    ...
    def place_market_order(self, trade_order: TradeOrder) -> bool:
        ...
        try:
            ...
            try:
                ...
                # Add weekend testing flag
                if self.weekend_testing.enable_weekend_testing:
                    airtable_fields['testing'] = 'WEEKEND_TEST'
    ...
    def get_strategy_analytics(self) -> Dict:
        ...
        try:
            ...
            for order_id, metadata in all_metadata.items():
                ...
                # Check for weekend testing trades
                if hasattr(metadata, 'notes') and metadata.notes and 'WEEKEND TESTING' in metadata.notes:
                    weekend_testing_count += 1
            ...
            return {
                ...
                'weekend_testing_trades': weekend_testing_count,
                'live_trades': len(all_metadata) - weekend_testing_count,
    ...
    def generate_trading_report(self) -> str:
        """Generate enhanced trading report with strategy analytics - FIXED TIMEZONE BUG"""
        try:
            ...
            if self.weekend_testing.enable_weekend_testing:
                report.append("DEMO TRADING BOT - WEEKEND TESTING MODE - ENHANCED WITH AIRTABLE")
                report.append("🧪 WEEKEND TESTING ACTIVE - Market timing bypassed for testing")
            else:
            ...
            current_et = self.get_market_time_et()
            tz_name = getattr(current_et, 'tz_name', 'EDT')  # Use the custom attribute we set
            report.append(f"Timestamp: {current_et.strftime('%Y-%m-%d %H:%M:%S')} {tz_name}")
            
            if self.weekend_testing.enable_weekend_testing:
                report.append(f"Testing Mode: 🧪 WEEKEND TESTING - {self.weekend_testing.simulated_session}")
                report.append(f"Market Open: ✅ Simulated (Always Open for Testing)")
            else:
            ...
            try:
                ...
                if 'error' not in strategy_analytics and 'message' not in strategy_analytics:
                    ...
                    if self.weekend_testing.enable_weekend_testing or strategy_analytics.get('weekend_testing_trades', 0) > 0:
                        report.append(f"   Weekend Testing Trades: {strategy_analytics.get('weekend_testing_trades', 0)}")
                        report.append(f"   Live Trades: {strategy_analytics.get('live_trades', 0)}")
            ...
            if self.weekend_testing.enable_weekend_testing:
                report.append("🧪 WEEKEND TESTING MODE - MARKET TIMING:")
                report.append(f"   Simulated Time: {session_info['current_time_et']}")
                report.append(f"   Simulated Session: {self.weekend_testing.simulated_session}")
                ...
                report.append(f"   Simulated Liquidity: {session_info['liquidity_level']}")
            else:
            ...
            timing_header = "📊 INSTRUMENT TIMING STATUS (Top 12)" + (" - WEEKEND TESTING" if self.weekend_testing.enable_weekend_testing else "")
            report.append(timing_header + ":")
            ...
            for instrument in main_instruments:
                try:
                    ...
                    if self.weekend_testing.enable_weekend_testing:
                        status = "🧪 TESTING" if is_optimal else "🧪 TESTING (suboptimal)"
                    else:
            ...
            report.append("")
            if self.weekend_testing.enable_weekend_testing:
                report.append("🧪 WEEKEND TESTING CONDITIONS:")
                report.append("   🧪 TESTING MODE - All market timing checks bypassed")
                report.append("   🧪 Confidence threshold lowered to 60% for testing")
                report.append("   🧪 All trades placed in practice account")
                report.append(f"   🧪 Simulated session: {self.weekend_testing.simulated_session}")
            else:
            ...
            report.append("")
            if self.weekend_testing.enable_weekend_testing:
                report.append("🧪 WEEKEND TESTING MODE FEATURES:")
                report.append(f"   ✅ Market timing checks bypassed for testing")
                report.append(f"   ✅ Simulated market sessions and liquidity")
                report.append(f"   ✅ Lower confidence thresholds (60%)")
                report.append(f"   ✅ All trades still placed in practice account")
                report.append(f"   ✅ Weekend testing flag in metadata")
                report.append("")
            ...
            report.append(f"   ✅ Weekend Testing: Market timing bypassed for weekend strategy testing")
            report.append(f"   ✅ Timezone Bug Fixed: Proper handling of custom tz_name attribute")
            report.append("")
            ...
            error_msg = f"Error generating trading report: {str(e)}"
            self.logger.error(error_msg)
            return f"Trading report unavailable: {error_msg}"
    
    def trading_cycle(self) -> None:
        """Main trading cycle - runs every 15 minutes"""
        try:
            mode_info = "🧪 WEEKEND TESTING" if self.weekend_testing.enable_weekend_testing else "LIVE"
            self.logger.info(f"🔄 Starting trading cycle ({mode_info})...")
            
            # Check if market is open (bypassed in weekend testing mode)
            ...
            self.logger.info(f"✅ Trading cycle completed ({mode_info})")
    ...
    def start_trading(self) -> None:
        ...
        mode_info = "WEEKEND TESTING MODE" if self.weekend_testing.enable_weekend_testing else "LIVE MODE"
        print(f"🚀 Starting Demo Trading Bot - {mode_info} - ENHANCED WITH AIRTABLE METADATA!")
        print("⏰ Trading every 15 minutes during market hours")
        print("⏹️  Press Ctrl+C to stop")
        
        if self.weekend_testing.enable_weekend_testing:
            print("🧪 Weekend testing active - market timing bypassed")
        ...
        try:
            ...
            print(f"\n🛑 Demo Trading Bot stopped ({mode_info})")
    
    def run_single_cycle(self) -> None:
        """Run a single trading cycle for testing"""
        mode_info = "WEEKEND TESTING MODE" if self.weekend_testing.enable_weekend_testing else "LIVE MODE"
        print(f"🧪 Running single trading cycle ({mode_info}) - ENHANCED WITH AIRTABLE METADATA...")
        self.trading_cycle()

def main():
    """Main function to run the demo trading bot"""
    print("🤖 Demo Trading Bot - ENHANCED WITH AIRTABLE METADATA + WEEKEND TESTING")
    print("="*80)
    ...
    # NEW: Weekend testing mode selection
    print("\n🧪 Testing Mode Options:")
    print("1. Live Mode: Normal market timing (weekdays only)")
    print("2. Weekend Testing: Bypass market timing for weekend strategy testing")
    
    try:
        test_choice = input("\nEnter choice (1-2) or press Enter for live mode: ").strip()
        
        if test_choice == "2":
            weekend_testing = True
            print("Selected: 🧪 Weekend Testing Mode")
            
            # Optional: Set specific testing session
            print("\n🕒 Simulate specific trading session? (optional)")
            print("1. London session (8 AM ET)")
            print("2. London-NY overlap (10 AM ET) - Recommended")
            print("3. Tokyo session (9 PM ET)")
            print("4. Custom time")
            print("5. Skip (use current time)")
            
            session_choice = input("\nEnter choice (1-5) or press Enter to skip: ").strip()
            
        else:
            weekend_testing = False
    ...
    try:
        ...
        mode_info = "weekend testing mode" if weekend_testing else "live mode"
        print(f"\n🎯 Initializing bot in {mode_info} with ${max_risk_usd:.2f} USD max risk per trade...")
        
        # Initialize bot
        bot = DemoTradingBot(
            max_risk_usd=max_risk_usd,
            max_open_trades=5,
            weekend_testing=weekend_testing
        )
        
        # Set up weekend testing session if selected
        if weekend_testing and session_choice:
            if session_choice == "1":
                bot.set_weekend_testing_session("london", 8)
            elif session_choice == "2":
                bot.set_weekend_testing_session("overlap", 10)
            elif session_choice == "3":
                bot.set_weekend_testing_session("tokyo", 21)
            elif session_choice == "4":
                try:
                    custom_hour = int(input("Enter hour (0-23 ET): "))
                    bot.set_weekend_testing_session("custom", custom_hour)
                except ValueError:
                    print("Invalid hour, using 10 AM ET default")
                    bot.set_weekend_testing_session("overlap", 10)
        ...
        if choice == "1":
            print(f"\n🧪 Running single test cycle ({mode_info})...")
            bot.run_single_cycle()
        elif choice == "2":
            print(f"\n⚡ Starting automated trading ({mode_info})...")
            bot.start_trading()
        else:
            print(f"\nInvalid choice. Running single test cycle ({mode_info})...")
            bot.run_single_cycle()
